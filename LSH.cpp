...
LSH::LSH(int kk, int window, int tablesize){ //CONSTRUCTOR
    k = kk;
    tableSize = tablesize;
    w = window;
    LSH_hashTable = new vector<bucketList>[tableSize];
    v_array = new vector<float>[k]; //ARRAY WITH K V
}
...
float LSH::inner_product(data* p, int i){//INNER PRODUCT V*P
...
vector<data> LSH::kNNsearch(vector<int> q, int d, int N){ //FIND N NEAREST NEIGHBORS
...
vector<data*> LSH::range_search(vector<int> q, int d, int minR, int maxR){ //RANGE SEARCH
