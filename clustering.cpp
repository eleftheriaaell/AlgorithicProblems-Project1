...
void Clustering::init_Push(data* p){//PLACE ALL THE POINTS TO THE FIRST CENTROID
    clusterList node;
...
void Clustering::Di_first_centroid(int d){//INITIALIZE THE Di(DISTANCE BETWEEN POINT AND CENTROID) FOR EACH POINT
    ...
    float max = distance(cluster_Table->begin()->p->p_data, centroid.p->p_data, d); //MAX DISTANCE BETWEEN CENTROID AND POINT
...
void Clustering::table_Push(vector<data*> dataList, int t){//ASSIGN ALL THE POINTS FROM DATALIST TO THE NEAREST CENTROID
    ...
    for(auto point: dataList){//FOR EACH POINT
        float min = distance(point->p_data, cluster_Table[0].begin()->p->p_data, point->d);
        int centroid = 0;
        
        float max = distance(point->p_data, cluster_Table[0].begin()->p->p_data, point->d);
        for(int i = 1; i < t; i++){//CHECK THE DISTANCE BETWEEN THE CURRENT POINT AND THE CENTROIDS
            float current = distance(point->p_data, cluster_Table[i].begin()->p->p_data, point->d);
...
int Clustering::binary_search(vector<probability> P_array, int n, float target){//BINARY SEARCH
...
void Clustering::table_Push_reverse(vector<data*> dataList, int t){//ASSIGNE THE POINTS TO CENTROIDS FOR THE REVERSE LSF/HYPERCUBE
...
void Clustering::reverse_range_search_LSH(LSH* objLSH, int L, int R, vector<data*> dataList){//REVERS RANGE SEARCH LSH
    ...
    while(iteration_flag < K){//STOP THE CENTROIDS FROM UPDATING IF THE DISTANCES BETWEEN THE CURRENT AND NEW CENTROIDS ITS ZERO
        ...
        while(radii_flag < K){//STOP INCREASING RANGE IF THE ALL CLUSTERS GET NO NEW POINT
            radii_flag = 0;
...
void Clustering::reverse_range_search_HYPER(HyperCube objHYPER, int R, vector<data*> dataList, int M, int probes){//REVERSE RANGE SEARCH WITH HYPERCUBE
    ...
    while(iteration_flag < K){
        ...
        while(radii_flag < K){
            ...
            for(int j = 0; j < K; j++){
                vector<data*> Rarr;
                Rarr = objHYPER.range_search(cluster_Table[j].front().p->p_data, cluster_Table[j].front().p->d, minR, maxR, M, probes);           
    ...
    for(int i = 0; i < K; i++){//FOR EACH CLUSTER
        for(auto x: cluster_Table[i]){//FOR EACH POINT IN CURRENT CLUSTER
            ...
            for(int j = 0; j < K; j++){
                ...
                float current = distance(x.p->p_data, cluster_Table[j].begin()->p->p_data, x.p->d);
                if(current < min){
                    min = current;
                    centroid = j;
                }
